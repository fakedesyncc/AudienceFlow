...
Рабочая web-консоль: https://fakedesyncc.github.io/AudienceFlow/work/
...
Web-клиент — `services/web`. Он собирается в двух независимых контурах:

- `demo` — публичная витрина для GitHub Pages с обезличенным презентационным потоком;
- `work` — рабочая web-консоль без demo-данных и без презентационного входа, только через реальный API.

Ни desktop, ни web не хранят рабочие секреты. В рабочем web-контуре пользователь вводит `API URL`, email и пароль; публичная demo-страница не связана с этой сессией и использует отдельное browser storage.
...
Импорт расписания поддерживает обычную нормализованную Excel-таблицу и формат ЛГТУ из файла `ikn-bak.xlsx`: несколько листов, колонки `Группа ...` / `Аудито рия`, переносы строк внутри ячеек, весенний/осенний семестр и расписание по дням недели.
...
flowchart LR
    ...
    API -->|"REST + WebSocket"| Web["React web work console"]
```
...
- рабочая web-консоль: тот же frontend в режиме `VITE_PUBLIC_CONTOUR=work`
