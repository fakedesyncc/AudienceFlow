...
AULA считает посещаемость не «внутри одного большого приложения», а через цепочку небольших сервисов. Так проще объяснять систему на защите и проще менять отдельные части: камеру, способ детекции, API или frontend.
