...
| `desktop-client` | Java / JavaFX | Основной рабочий клиент для Windows, macOS и Linux: live-видео, zoom, снимки, мониторинг |
...
sequenceDiagram
    ...
    G->>P: batch insert
    D->>W: preview frame/state
    W-->>D: JPEG frame + detections
    D->>A: REST запросы
    A->>P: SQL агрегаты
    A-->>D: JSON
...
Видео не прокидывается через Analytics API: desktop подключается к preview endpoint конкретного `vision-worker` и получает уже размеченный JPEG/MJPEG поток. Так API не становится видеопрокси, а события посещаемости остаются в обычном контуре `worker → Go → PostgreSQL → Java API`.
