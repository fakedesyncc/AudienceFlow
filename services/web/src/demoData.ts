...
export function fetchDemoCurrent(): CurrentAttendance[] {
...
export function fetchDemoTimeline(roomId: number): TimelinePoint[] {
  const now = Date.now();
...
export function fetchDemoRooms(): Room[] {
  return demoRooms;
}

export function createDemoRoom(payload: CreateRoomPayload): Room {
  const room = { ...payload, id: nextId(demoRooms) };
  demoRooms = [...demoRooms, room];
  return room;
}

export function fetchDemoCameras(): Camera[] {
  return demoCameras.map((camera) => ({ ...camera, sourceUrl: null }));
}

export function createDemoCamera(payload: CreateCameraPayload): Camera {
...
export function updateDemoCamera(id: number, payload: CreateCameraPayload): Camera {
  const room = demoRooms.find((candidate) => candidate.id === payload.roomId);
...
export function fetchDemoCampusBuildings(): CampusBuilding[] {
  return demoBuildings;
}

export function fetchDemoScheduleDirectory(): ScheduleDirectory {
  return demoDirectory;
}

export function fetchDemoSchedule(
  date: string,
  params: { buildingId?: number | null; teacherId?: number | null; groupId?: number | null },
): ScheduleEntry[] {
  return demoSchedule(date).filter((entry) => {
...
export function fetchDemoScheduleAnalytics(
  date: string,
...
export function verifyDemoTeacherAccess(teacherId: number, accessKey: string): TeacherAccessVerification {
...
export function issueDemoTeacherAccessKey(teacherId: number): TeacherKeyIssueResponse {
  const teacher = demoDirectory.teachers.find((item) => item.id === teacherId);
  if (!teacher) {
...
export function fetchDemoUsers(): UserView[] {
  return [];
}
